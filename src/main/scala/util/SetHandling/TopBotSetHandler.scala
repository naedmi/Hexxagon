...
class TopBotSetHandler() extends DefaultSetHandler {
    ...
    override def createSetandHandle(content: Char, x: Int, y: Int, startmatrix: Vector[Vector[Char]]) = {
        ...
        setMaxY(startmatrix)
        Try(setforeach(Set((x, y + 1), (x - 1, 0), (x - 1, ibound + 1), (x + 1, 0), (x + 1, ibound + 1)), tmpmatrix, content)) match {
            case Success(o) => o
            case Failure(_) => {
                Try(setforeach(Set((x, y - 1), (x - 1, MaxY), (x - 1, ibound), (x + 1, MaxY), (x + 1, ibound)), tmpmatrix, content)) match {
                    case Success(o) => o
